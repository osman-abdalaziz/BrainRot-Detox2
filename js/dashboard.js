...
function updateProfileUI(userData) {
    ...
    if (freezeEl) {
        if (hasFreeze) freezeEl.style.display = "inline-block";
...
async function loadLeaderboard() {
    ...
    try {
        ...
        // 2. خوارزمية الفرز الجديدة (الستريك الحالي -> XP الحالي -> الترتيب الأبجدي)
        usersArray.sort((a, b) => {
            ...
            if (xpA !== xpB) return xpB - xpA; // عند تعادل الستريك، الأولوية للـ XP
        ...
        usersArray.forEach((user) => {
            // التحقق من التعادل مع الشخص السابق (تم تصحيح أسماء المتغيرات)
            if (previousUser) {
                const sameStreak =
            ...
            // 1. استخراج تفاصيل اللقب بناءً على السكور التراكمي
            ...
            //         <div class="avatar-wrapper ${frameClass}" style="width: 48px; height: 48px;">
            ...
            //         ${streakHtml}
            //         <span class="task-points-badge">${user.currentXP || 0} XP</span>
            //     </div>
            // `;
            userDiv.innerHTML = `
                <div class="leaderboard-user-info" style="min-width: 0;"><div class="${rankClass}">#${currentRank}</div>
                    <div class="avatar-wrapper ${frameClass}" style="width: 45px; height: 45px; flex-shrink: 0;">
                        <img src="${user.photoURL || "images/profile.jpg"}" alt="Avatar">
                    </div>
                    <div style="display: flex; flex-direction: column; gap: 3px; min-width: 0;">
                        <span class="leaderboard-name" style="${nameGlow}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${user.name}</span>
                        <span class="rank-tag ${rankInfo.tagClass}">${rankInfo.title}</span>
                    </div>
                </div>
